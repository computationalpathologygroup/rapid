...
from keypoints import divide_image_in_patches
...
def find_dorsal_rotation(mask: np.ndarray, ellipse: Any, center: np.ndarray, savepath: pathlib.Path) -> int:
    ...
    _, mask, _ = apply_affine_transform(
    ...
    # Find bbox corners and angle
    bbox = cv2.minAreaRect(contour)
    bbox_corners = cv2.boxPoints(bbox)
    bbox_angle = bbox[-1]
    bbox_angle = bbox_angle-90 if bbox_angle > 45 else bbox_angle

    # Get long edges of bounding box
    ...
    # First rotation to make dorsal side face downward
    dorsal_rotation = 180 if lower_corner_dist > upper_corner_dist else 0
    
    # Second rotation to make long edge parallel to x-axis
    # final_rotation = dorsal_rotation + bbox_angle
    final_rotation = dorsal_rotation

    # Apply final rotation to mask for visualization
    rotation_matrix = cv2.getRotationMatrix2D(tuple(center), final_rotation, 1)
    _, mask_rotated, _ = apply_affine_transform(
        image = np.zeros((mask.shape[0], mask.shape[1], 3)),
        tform = rotation_matrix,
        mask = mask,
    )

    # Apply rotation to bbox corners and contour
    ones = np.ones(shape=(len(upper_bbox_corners),1))
    upper_bbox_corners_rotated = np.dot(rotation_matrix, np.hstack([upper_bbox_corners, ones]).T).T
    lower_bbox_corners_rotated = np.dot(rotation_matrix, np.hstack([lower_bbox_corners, ones]).T).T
    
    contour_ones = np.ones(shape=(len(contour),1))
    contour_rotated = np.dot(rotation_matrix, np.hstack([contour, contour_ones]).T).T

    colours = ["g", "r"] if dorsal_rotation == 180 else ["r", "g"]

    # Sanity check plot
    plt.figure()
    plt.imshow(mask_rotated, cmap="gray")
    plt.plot(contour_rotated[:, 0], contour_rotated[:, 1], "b")
    plt.scatter(upper_bbox_corners_rotated[:, 0], upper_bbox_corners_rotated[:, 1], c=colours[0])
    plt.scatter(lower_bbox_corners_rotated[:, 0], lower_bbox_corners_rotated[:, 1], c=colours[1])
    plt.title(f"Rotate {final_rotation:.1f} degrees.")
