...
def plot_keypoint_pairs(ref_image: np.ndarray, moving_image: np.ndarray, ref_points: List, moving_points: List, scores: List, tform: str, ransac_thres: float, savepath: pathlib.Path, filter_method: str = "none") -> None:
    """
    Function to plot the keypoint pairs on two images.
    """

    # Compute ransac matches to visualize the effect of RANSAC
    if tform == "affine" and filter_method == "ransac":
        ref_points_ransac, moving_points_ransac, inliers, _ = apply_affine_ransac(
            moving_points = moving_points,
            ref_points = ref_points,
            image = moving_image,
            ransac_thres = ransac_thres
        )
    elif tform == "affine" and filter_method == "magsac":
        ...
        ref_points_ransac = ref_points
        moving_points_ransac = moving_points
    ...
    if filter_method == "ransac":
        ...
        plt.title(f"MAGSAC matches: (n={len(ransac_matches)})")
    plt.axis("off")
    plt.tight_layout()
    plt.savefig(savepath, dpi=300, bbox_inches="tight")
    plt.close()
...
def plot_dino_features(ref_image: np.ndarray, moving_image: np.ndarray, ref_mask: np.ndarray, moving_mask: np.ndarray, ref_dino_features: np.ndarray, moving_dino_features: np.ndarray, cosine_similarity: float, savepath: pathlib.Path) -> None:
    """
    Function to plot the DINO fx.
    """
    ...
    # Reshape to 2D
    ref_flat = ref_dino_features.reshape(-1, ref_dino_features.shape[-1])
    moving_flat = moving_dino_features.reshape(-1, moving_dino_features.shape[-1])
    
    # Mask bg features
    ref_flat = mask_bg_features(ref_flat, ref_mask)
    moving_flat = mask_bg_features(moving_flat, moving_mask)
    
    # Fit PCA on concatenated features for consistent embedding
    pca = PCA(n_components=3)
    pca.fit(np.vstack([ref_flat, moving_flat]))
    
    # Transform separately
    pca_ref = pca.transform(ref_flat)
    pca_mov = pca.transform(moving_flat)

    # Normalize PCA features to 0-1 range
    scaler = MinMaxScaler(feature_range=(0, 1), clip=True)
    for i in range(3):
        pca_ref[:, i] = scaler.fit_transform(pca_ref[:, i].reshape(-1, 1)).reshape(-1)
        pca_mov[:, i] = scaler.fit_transform(pca_mov[:, i].reshape(-1, 1)).reshape(-1)

    # Reshape feature vector back to image shape for plotting
    pca_ref = pca_ref.reshape(ref_dino_features.shape[0], ref_dino_features.shape[1], 3)
    pca_mov = pca_mov.reshape(moving_dino_features.shape[0], moving_dino_features.shape[1], 3)

    # Mask out any background artefacts
    ref_bg = cv2.resize(ref_mask, (pca_ref.shape[0], pca_ref.shape[1]))
    moving_bg = cv2.resize(moving_mask, (pca_mov.shape[0], pca_mov.shape[1]))
    pca_ref[ref_bg == 0] = 0
    pca_mov[moving_bg == 0] = 0

    plt.figure()
    plt.suptitle(f"cosine similarity: {cosine_similarity:.4f}")
    for i, (title, im) in enumerate([("ref", ref_image), ("mov", moving_image), ("ref", pca_ref), ("mov", pca_mov)]):
        plt.subplot(2, 2, i+1)
        plt.title(title)
        plt.imshow(im)
        plt.axis("off")
...
def plot_warped_images(ref_image: np.ndarray, ref_mask: np.ndarray, moving_image: np.ndarray, moving_image_warped: np.ndarray, moving_mask_warped: np.ndarray, savepath: pathlib.Path) -> None:
    ...
    # Compute overlap
    overlap = compute_dice(moving_mask_warped, ref_mask, normalized=True)
...
def plot_warped_deformable_images(ref_image: np.ndarray, ref_mask: np.ndarray, moving_image: np.ndarray, moving_mask: np.ndarray, moving_image_warped: np.ndarray, moving_mask_warped: np.ndarray, grid: Any, moving_landmarks: np.ndarray, moving_landmarks_warped: np.ndarray, savepath: pathlib.Path) -> None:
    """
    ...
    # Get contours for visualization purposes
    if len(np.unique(moving_mask_warped)) > 1:
        cnt_moving_warped, _ = cv2.findContours(moving_mask_warped, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
        cnt_moving_warped = np.squeeze(max(cnt_moving_warped, key=cv2.contourArea))
    else:
        moving_mask_warped[0, 0] = 1
        cnt_moving_warped = np.zeros((2, 2))
        
    if len(np.unique(moving_mask)) > 1:
        cnt_moving, _ = cv2.findContours(moving_mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
        cnt_moving = np.squeeze(max(cnt_moving, key=cv2.contourArea))
    else:
        moving_mask[0, 0] = 1
        cnt_moving = np.zeros((2, 2))
    ...
    overlap_pre = compute_dice(moving_mask, ref_mask, normalized=True)
    overlap_post = compute_dice(moving_mask_warped, ref_mask, normalized=True)
    ...
    plt.subplot(162)
    plt.imshow(moving_image)
    if moving_landmarks is not None:
        plt.scatter(moving_landmarks[:, 0], moving_landmarks[:, 1], c="r", s=2)
    plt.axis("off")
    plt.title("mov")
    plt.subplot(163)
    ...
    plt.subplot(164)
    plt.imshow(moving_image_warped)
    if moving_landmarks_warped is not None:
        plt.scatter(moving_landmarks_warped[:, 0], moving_landmarks_warped[:, 1], c="r", s=2)
    plt.axis("off")
    plt.title("mov'")
    plt.subplot(165)
    plt.imshow(np.zeros_like(moving_image_warped))
    ...
    plt.scatter(cnt_moving[:, 0], cnt_moving[:, 1], c="b", s=2)
    plt.axis("off")
    plt.title(f"dice: {overlap_pre:.2f}")
    plt.subplot(166)
    plt.imshow(np.zeros_like(moving_image_warped))
    plt.scatter(cnt_ref[:, 0], cnt_ref[:, 1], c="r", s=2)
    plt.scatter(cnt_moving_warped[:, 0], cnt_moving_warped[:, 1], c="b", s=2)
    plt.axis("off")
    plt.title(f"dice: {overlap_post:.2f}")
    plt.savefig(savepath, dpi=300, bbox_inches="tight")
    plt.close()
...
def plot_tre_per_pair(ref_image: np.ndarray, moving_image: np.ndarray, ref_points: np.ndarray, moving_points: np.ndarray, scores: List, tre: float, savepath: pathlib.Path) -> None:
    ...
    if ref_points.shape[0] > 500:
        original_n = ref_points.shape[0]
        subset = np.arange(0, ref_points.shape[0], ref_points.shape[0]//500)
        ref_points = ref_points[subset]
        moving_points = moving_points[subset]

    plt.figure(figsize=(8, 4))
    if subset is not None:
        plt.suptitle(f"TRE: {tre:.2f} microns (n={len(ref_points)}/{original_n})")   
    else:
...
def plot_tre_landmarks(images: List, landmarks: List, tre: float, save_path: pathlib.Path) -> None:
    """
    Function to plot the TRE per pair of images.
    """
    
    plt.figure(figsize=(8, 4))
    plt.suptitle(f"TRE: {tre:.2f} px")
    plt.subplot(131)
    plt.imshow(images[0])
    plt.scatter(landmarks[0][:, 0], landmarks[0][:, 1], c="r", s=5)
    plt.title("im1")
    plt.axis("off")
    plt.subplot(132)
    plt.imshow(images[1])
    plt.scatter(landmarks[1][:, 0], landmarks[1][:, 1], c="b", s=5)
    plt.title("im2")
    plt.axis("off")
    plt.subplot(133)
    plt.imshow(np.zeros_like(images[0]))
    plt.scatter(landmarks[0][:, 0], landmarks[0][:, 1], c="r", s=5)
    plt.scatter(landmarks[1][:, 0], landmarks[1][:, 1], c="b", s=5)
    plt.legend(["im1", "im2"])
    plt.axis("off")
...
def plot_orientation_accuracy(final_images: List, deltas: List, save_dir: pathlib.Path) -> None:
    """
    Function to plot the orientation accuracy.
    """

    # Get path to save
    idx = get_save_image_idx(save_dir)
    savepath = save_dir.joinpath(f"{str(idx).zfill(2)}_orientation_accuracy.png")

    plt.figure(figsize=(10, 5))
    for c, (im, delta) in enumerate(zip(final_images, deltas), 1):
        plt.subplot(1, len(final_images), c)
        plt.imshow(im)
        plt.title(f"delta rot: \n{delta:.1f} deg")
        plt.axis("off")
    plt.tight_layout()
    plt.suptitle("Orientation accuracy")
    plt.savefig(savepath, dpi=300, bbox_inches="tight")
    plt.close()
